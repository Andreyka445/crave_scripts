#!/bin/bash
set -e

crave run --no-patch -- "
rm -rf .repo/local_manifests &&
rm -rf device/tecno/LH7n &&
rm -rf device/tecno/mt6789-common &&
rm -rf device/tecno/LH7n-kernel &&
rm -rf vendor/tecno/LH7n &&
rm -rf vendor/tecno/mt6789-common &&
rm -rf vendor/sony/dolby &&
rm -rf vendor/JamesDSP &&
rm -rf packages/apps/ViPER4AndroidFX &&
rm -rf hardware/mediatek &&
rm -rf hardware/transsion &&
rm -rf device/mediatek/sepolicy_vndr &&
rm -rf vendor/lineage-priv/keys &&
rm -rf vendor/evolution-priv/keys &&
rm -rf vendor/clover-priv/keys &&
rm -rf device/qcom/sepolicy_vndr &&
rm -rf build/soong &&
rm -rf vendor/google/gms &&
rm -rf vendor/gms &&
rm -rf prebuilts/clang/host/linux-x86 &&
rm -rf platform/prebuilts/clang/host/linux-x86 &&

echo "==> Syncing sources..."

#Clone the deivce manifest
git clone https://github.com/Andreyka445/local_manifests.git -b clover-16-lh7n .repo/local_manifests

#initialize rom repo
repo init -u https://github.com/The-Clover-Project/manifest.git -b 16-qpr1 --git-lfs

#Sync
/opt/crave/resync.sh

#Signing
git clone --depth=1 --branch clover https://github.com/Andreyka445/signingkey vendor/clover-priv/keys

echo "=== Starting Build ==="

#Setup environment and start build
. build/envsetup.sh
lunch clover_LH7n-bp3a-userdebug
make installclean
mka clover
"
